...
Please run again with spin-unpolarized input."
    ...
    def calc_sig_func(
        ...
        sig_omega = arrays.zeros((np.size(omega_arr), 2))
        omega_dummy_mat = arrays.ones((nbands, lmax, nmax, lmax, nmax, n_freq))
    ...
    def calc_occ_diff_mat(occnums, orb_subset_1, orb_subset_2):
        ...
        occ_diff_mat = arrays.zeros((nbands, lmax, nmax, lmax, nmax), dtype=np.float32)
    ...
    def calc_eig_diff_mat(eigvals, orb_subset_1, orb_subset_2):
        ...
        eig_diff_mat = arrays.zeros((nbands, lmax, nmax, lmax, nmax), dtype=np.float32)
        eig_diff_mat += 1e-6
...
class SphHamInts:
    ...
    def P_mat_int(cls, func_int, lmax):
        ...
        """
        P_mat = arrays.zeros((lmax, lmax, 2 * lmax + 1))
...
class RadialInts:
    ...
    def calc_R1_int_mat(cls, eigfuncs, occnums, xgrid, orb_subset_1, orb_subset_2):
        ...
        nbands, nspin, lmax, nmax = np.shape(occnums)
        R1_mat = arrays.zeros((nbands, lmax, nmax, lmax, nmax), dtype=np.float32)
    ...
    def calc_R2_int_mat(cls, eigfuncs, occnums, xgrid, orb_subset_1, orb_subset_2):
        ...
        # initiliaze the matrix
        nbands, nspin, lmax, nmax = np.shape(occnums)
        R2_mat = arrays.zeros((nbands, lmax, nmax, lmax, nmax), dtype=np.float32)
