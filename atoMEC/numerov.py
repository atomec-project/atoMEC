...
class Solver:
    ...
    def matrix_solve(self, v, xgrid, bc, solve_type="full", eigs_min_guess=None):
        ...
        if solve_type == "guess":
            ...
            dtype = dtype_map[config.float_precision]
    ...
    def update_orbs(l_eigfuncs, l_eigvals, xgrid, bc, K, grid_type):
        ...
        eigfuncs = arrays.array(np.transpose(l_eigfuncs.real)[idr])
        if grid_type == "sqrt":
            eigfuncs *= xgrid**-1.5
    ...
    def calc_wfns_e_grid(self, xgrid, v, e_arr, eigfuncs_l, eigfuncs_u):
        ...
        W_arr = arrays.zeros((N, nkpts, spindims, lmax, nmax))
        eigfuncs_init = np.zeros_like(W_arr)
        eigfuncs_backup = arrays.zeros((nkpts, spindims, lmax, nmax, N))
